...
class Transactions(UserMixin, db.Model):
    ...
    '''
        Transactions table to store transactions made by a user
    '''
...
class ItemList(UserMixin, db.Model):
    ...
    '''
        Item table to store items belonging to specific transaction
    '''
...
class MenuAdd(Resource):
    def post(self):
        data_posted_by_user = request.get_json()
        username = data_posted_by_user['username']
        if User.query.filter_by(username=username).first().is_chef:
            data_posted_by_user = request.get_json()
    ...
    def get(self):
        ...
        for menu in data:
            returnValue[menu.item_id] = {
                                        "half_plate_price": menu.half_plate_price,
                                        "full_plate_price": menu.full_plate_price
                                        }
        return jsonify(returnValue)
...
class TransactionView(Resource):
    def post(self):
        ...
        returnValue = {}
        for tx in data:
            returnValue[tx.transaction_id] = {
                "total_bill_cost": tx.total_bill_cost}
        return jsonify(returnValue)
...
class TransactionSpecificView(Resource):
    def post(self):
        ...
        transaction_summary = Transactions.query.filter_by(
